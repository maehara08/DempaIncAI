...
def detect_face_rotate(img_file, base_dir, new_image_name, out_dir='out'):
    filename = os.path.basename(os.path.normcase(img_file))
    (fn, ext) = os.path.splitext(filename)
    # img_dir = '%s/%s/%s' % (base_dir, out_dir, fn)
    img_dir = '%s/%s' % (base_dir, out_dir)
    ...
    for deg in range(-50, 51, 5):
        ...
        # """
        # out_file = '%s/%s_deg_%s.jpg' % (img_dir, new_image_name, deg)
        # print out_file
        ...
        for (x, y, w, h) in faces:
            ...
            # """
            # out_file = '%s/face_%s_%s.jpg' % (img_dir, result['face_id'], deg)
...
if __name__ == "__main__":
    param = sys.argv
    detect_face_rotate(param[1], base_dir, "01", param[2])
